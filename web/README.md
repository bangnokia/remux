...
The installer downloads the latest server release, verifies its SHA-256 checksum, installs the CLI into `~/.telemux`, and creates a user systemd service when available. It uses the `node` already on the installing user's `PATH`.
