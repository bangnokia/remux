...
- Installs `tmux`, `curl`, and `tar` where possible.
...
- Creates and starts a user `telemux` systemd service when systemd is available.

It uses the `node` already on the installing user's `PATH`, including user-installed Node managers such as `fnm`. It may ask for your sudo password when base packages need root access or when removing an older root systemd service from a previous installer version.
...
Install without creating a user systemd service:
...
When systemd is available, the installer creates `~/.config/systemd/user/telemux.service` and starts it automatically. Check it with:

```bash
systemctl --user status telemux
```

View logs or restart after changing config:

```bash
journalctl --user -u telemux -f
systemctl --user restart telemux
```
